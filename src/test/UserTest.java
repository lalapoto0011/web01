...
class UserTest {
	...
	UserService service = null; //service;는 service=null;이랑 같음..
	//주소가 들어가야되니까 null을 줌 ????
	...
	void setUp() throws Exception {
		...
		service = new UserServiceImpl(dao); //impl이 만들어짐
	...
	//	service.bookList().forEach(i -> {System.out.println(i);} ); //디비에 있는 북리스트 들어옴
		...
		List<UserVO> list = service.userList(); //bookList 결과값이 list로 들어감
		
		for(UserVO data : list) {
			System.out.println("------------------");
			System.out.printf("%s|%d|%s %n", data.getId(), data.getName(), data.getRole());
			System.out.println("------------------");
			//웹이라면 테이블 구조에 담음
		}
	}
